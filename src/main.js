...
function showTab(name){
  ...
  ['invullen','resultaat'].forEach((t,i)=>{
    if(t===name){ const tabs=document.querySelectorAll('.tab'); if(tabs[i]) tabs[i].classList.add('active'); }
  });
...
function getClubLogoFromMap(name){
  if(!name) return '';
...
function getClubLogo(name){
  return getClubLogoFromMap(name);
}

function teamLogoImg(name, style=''){
  if(!name) return '';
  const local = `/logo/${encodeURIComponent(name.trim())}.png`;
...
function syncModeLabels(){
  ...
  if(wl){
    wl.style.background = landen ? 'var(--oranje-dim)' : 'rgba(255,255,255,.04)';
    wl.style.borderColor = landen ? 'var(--border-orange)' : 'var(--border)';
    wl.style.opacity = landen ? '1' : '0.45';
  }
  if(wc){
    wc.style.background = !landen ? 'var(--oranje-dim)' : 'rgba(255,255,255,.04)';
    wc.style.borderColor = !landen ? 'var(--border-orange)' : 'var(--border)';
...
function renderInvullen(){
  ...
  if(editingPlayer){
    // Force exit editing mode when locked (for non-admins)
    if(state.locked && !isAdmin){
      editingPlayer = null;
    } else {
      state.activePlayer = editingPlayer;
      // Never re-render the form during a Supabase realtime event — avoids keyboard dismissal
      if(_realtimeUpdate) return;
      renderInvullenForm();
      return;
    }
  }
...
function saveLocalVoorspelling(){
  ...
  state.vragen.forEach(v=>{
    if(v.type==='score'||v.type==='tussenstand'){
      ...
      const e2=document.getElementById('pred_'+v.id+'_2');
      if(e1||e2){
        const v1=e1?e1.value.trim():''; const v2=e2?e2.value.trim():'';
      ...
      const el=document.getElementById('pred_'+v.id);
      if(el) pred[v.id]=el.value;
      if(v.type==='jn'&&state.strafMode){ const subEl=document.getElementById('pred_'+v.id+'_s'); if(subEl) pred[v.id+'_s']=subEl.value||''; }
    }
  });
}

function checkDependentRender(vraagId){
  const hasDependents = state.vragen.some(v => v.dependsOn && v.dependsOn.id === vraagId);
  if(hasDependents){ saveLocalVoorspelling(); renderInvullenForm(); }
}
